      ...
      {results.map((result) => (
        <article key={result.id} className="rounded-[28px] border border-black/5 bg-white/80 p-5 shadow-panel">
          ...
          <div className="mt-4 grid gap-4 lg:grid-cols-2">
            ...
            <div>
              <h4 className="font-semibold text-ink">Model Output</h4>
              <pre className="mt-2 whitespace-pre-wrap rounded-2xl bg-slate-50 p-4 text-sm">{result.output}</pre>
            </div>
          </div>
          <div className="mt-4 grid gap-3 md:grid-cols-3">
            {result.scores.map((score) => (
              <div key={score.type} className="rounded-2xl border border-slate-100 bg-white p-4">
                ...
                <p className="mt-2 text-sm text-slate-600">{JSON.stringify(score.metadata)}</p>
              </div>
            ))}
          </div>
        </article>
